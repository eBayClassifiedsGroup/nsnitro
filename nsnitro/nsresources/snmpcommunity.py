...
class SNMPCommunity(NSBaseResource):
    ...
    def get(nitro, snmpcommunity):
        ...
        __snmpcommunity.get_resource(
            nitro, object_name=__snmpcommunity.get_communityname())
        return __snmpcommunity

    @staticmethod
    def get_all(nitro):
        __url = nitro.get_url() + SNMPCommunity.get_resourcetype()
        __json_snmpcommunities = nitro.get(__url).get_response_field(
            SNMPCommunity.get_resourcetype())
        __snmpcommunities = []
