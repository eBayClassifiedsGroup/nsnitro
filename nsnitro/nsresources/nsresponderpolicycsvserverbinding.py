...
class NSResponderPolicyCSVServerBinding(NSBaseResource):
    ...
    def get(nitro, responderpolicy_csvserver_binding):
        ...
        """
        __url = nitro.get_url() + NSResponderPolicyCSVServerBinding.get_resourcetype() + \
            "/" + responderpolicy_csvserver_binding.get_name()
        __json_resources = nitro.get(__url).get_response_field(
            NSResponderPolicyCSVServerBinding.get_resourcetype())
        __resources = []
        for json_resource in __json_resources:
            __resources.append(
                NSResponderPolicyCSVServerBinding(json_resource))
        return __resources
