...
class NSSSLCertKeySSLVServerBinding(NSBaseResource):
    ...
    def get(nitro, sslcertkey):
        ...
        """
        __url = nitro.get_url(
        ) + NSSSLCertKeySSLVServerBinding.get_resourcetype() + "/" + sslcertkey.get_certkey()
        __json_bindings = nitro.get(__url).get_response_field(
            NSSSLCertKeySSLVServerBinding.get_resourcetype())
        __bindings = []
