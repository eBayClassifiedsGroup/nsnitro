...
class SystemGlobalAuthTacacsPolicyBinding(NSBaseResource):
    ...
    def get_all(nitro):
        ...
        """
        __url = nitro.get_url(
        ) + SystemGlobalAuthTacacsPolicyBinding.get_resourcetype()
        __json_policybindings = nitro.get(__url).get_response_field(
            SystemGlobalAuthTacacsPolicyBinding.get_resourcetype())
        __policybindings = []
        for json_policybinding in __json_policybindings:
            __policybindings.append(
                SystemGlobalAuthTacacsPolicyBinding(json_policybinding))
