...
class NSServiceGroupServerBinding(NSBaseResource):
    ...
    def get(nitro, servicegroup_server_binding):
        ...
        """
        __url = nitro.get_url() + NSServiceGroupServerBinding.get_resourcetype() + \
            "/" + servicegroup_server_binding.get_servicegroupname()
        __json_services = nitro.get(__url).get_response_field(
            NSServiceGroupServerBinding.get_resourcetype())
        __services = []
    ...
    def get_server(nitro, servicegroup_server_binding):
        ...
        __servicegroup_server_binding.set_servicegroupname(
            servicegroup_server_binding.get_servicegroupname())
        __servicegroup_server_binding.set_servername(
            servicegroup_server_binding.get_servername())
        __filter = servicegroup_server_binding.get_servicegroupname(
        ) + "?filter=servername:" + servicegroup_server_binding.get_servername()
    ...
    def add(nitro, servicegroup_server_binding):
        ...
        __servicegroup_server_binding.set_servername(
            servicegroup_server_binding.get_servername())
        __servicegroup_server_binding.set_ip(
            servicegroup_server_binding.get_ip())
        __servicegroup_server_binding.set_port(
            servicegroup_server_binding.get_port())
        __servicegroup_server_binding.set_servicegroupname(
            servicegroup_server_binding.get_servicegroupname())
        __servicegroup_server_binding.set_weight(
            servicegroup_server_binding.get_weight())
        __servicegroup_server_binding.set_serverid(
            servicegroup_server_binding.get_serverid())
        __servicegroup_server_binding.set_state(
            servicegroup_server_binding.get_state())
        __servicegroup_server_binding.set_hashid(
            servicegroup_server_binding.get_hashid())
